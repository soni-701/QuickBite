...
export const createOwnerRestaurant=async(req:AuthRequest,res:Response):Promise<void> =>{
    ...
    try {
        ...
        if(!existing){
            res.status(200).json({message:"You already have a resturant registered"});
            return;
        }
...
//get booking for owner's restaurant (submitted to pending)
...
export const updateBookingStatus=async(req:AuthRequest,res:Response):Promise<void> =>{
    ...
    try {
        ...
        if(!restaurant || restaurant.owner.toString()!==req.user?._id.toString()){
            res.status(400).json({message:"Not authorised to manage this booking"})
            return;
        }
